...
|-------|---------|------------|
| `.name(name)` | stable authored name, unique within the owning assembly occurrence | all types |
...
For viewer animation, name the driver explicitly:
`mate('revolute', a, b).name('crank-drive')`. The viewer can select it with
`#animate=crank-drive&autoplay=1`. Names are case-sensitive and trimmed;
empty names and duplicates within one occurrence are rejected. Repeated
subassemblies may reuse local names; select an occurrence with `animate-owner`.
Mates generated by `replicate()` in the same scope do not inherit the seed's
name; mates authored inside each inserted subassembly keep their local names.
