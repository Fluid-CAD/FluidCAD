...
export type ExtrudeValues =
  | { op: FeatureOp; distance: number | null; thin: [number] | null }
  | { error: string };
...
export class ExtrudePanel {
  ...
  constructor(container: HTMLElement) {
    ...
    this.distanceInput.addEventListener('input', () => this.onChange?.());
  ...
  values(): ExtrudeValues {
    ...
    const throughAll = op === 'remove' && this.throughCheckbox.checked;
    ...
    return { op, distance, thin: thin.thin };
  }
