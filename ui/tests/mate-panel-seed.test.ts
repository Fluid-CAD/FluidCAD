...
describe('MatePanel origin-axis row', () => {
  it('is hidden on show(), toggles with setOriginAxisRow, and round-trips the axis', () => {
    const { panel, container } = openPanel();
    panel.show('revolute');
    const row = container.querySelector<HTMLElement>('[data-role="origin-axis-row"]')!;
    expect(row.classList.contains('hidden')).toBe(true);
    expect(panel.getOriginAxis()).toBe('z');

    panel.setOriginAxisRow(true, 'x');
    expect(row.classList.contains('hidden')).toBe(false);
    expect(panel.getOriginAxis()).toBe('x');

    // Visibility-only calls keep the chosen axis.
    panel.setOriginAxisRow(true);
    expect(panel.getOriginAxis()).toBe('x');

    panel.setOriginAxisRow(false);
    expect(row.classList.contains('hidden')).toBe(true);
  });

  it('resets to hidden and Z on the next show(), and hides for tangent', () => {
    const { panel, container } = openPanel();
    panel.show('revolute');
    panel.setOriginAxisRow(true, 'y');
    panel.show('revolute');
    const row = container.querySelector<HTMLElement>('[data-role="origin-axis-row"]')!;
    expect(row.classList.contains('hidden')).toBe(true);
    expect(panel.getOriginAxis()).toBe('z');
    ...
    panel.setOriginAxisRow(true, 'x');
    panel.setType('tangent');
    expect(row.classList.contains('hidden')).toBe(true);
  });

  it('a pen-less chip renders without the edit button', () => {
    const { panel, container } = openPanel();
    panel.show('revolute');
    panel.setSlotChip('a', 'Origin', { pen: false });
    panel.setSlotChip('b', 'arm1 · hinge');
    const slotA = container.querySelector('[data-role="slot-a"]')!;
    const slotB = container.querySelector('[data-role="slot-b"]')!;
    expect(slotA.textContent).toContain('Origin');
    expect(slotA.querySelectorAll('button[title*="propert"]').length).toBe(0);
    expect(slotB.querySelectorAll('button[title*="propert"]').length).toBe(1);
  });
});
