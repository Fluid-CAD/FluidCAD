...
2. The Revolve dialog:
   ...
   - **Axis** — click one of the world axes shown in the viewport (X red, Y green, Z blue), an `axis()` feature, or a straight edge. The slot opens on world Z. One slot is armed at a time (the highlighted border); viewport picks go to the armed slot.
