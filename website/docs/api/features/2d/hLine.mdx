---
sidebar_position: 10
title: "hLine"
description: "Draws a horizontal line of the given distance. Chain `.centered()` to center the line on the current position."
---

# hLine()

Draws a horizontal line of the given distance.
Chain `.centered()` to center the line on the current position.

**Returns**: [**`HLine`**](/docs/api/types/hline)

## Signatures

```ts
hLine(distance: NumberParam): HLine
```

Draws a horizontal line of the given distance.
Chain `.centered()` to center the line on the current position.

| Parameter | Type | Description |
|-----------|------|-------------|
| `distance` | NumberParam | The line length |

---

```ts
hLine(target: SceneObject): HLine
```

Draws a horizontal line that ends where it intersects the target geometry.
The nearest intersection (in either direction along the X axis) is used.

| Parameter | Type | Description |
|-----------|------|-------------|
| `target` | [SceneObject](/docs/api/types/scene-object) | The geometry to intersect with |

---

```ts
hLine(start: Point2DLike, distance: NumberParam): HLine
```

Draws a horizontal line from a start point.
Chain `.centered()` to center the line on the start point.

| Parameter | Type | Description |
|-----------|------|-------------|
| `start` | [Point2DLike](/docs/api/types/point2dlike) | The start point |
| `distance` | NumberParam | The line length |

---

```ts
hLine(start: Point2DLike, target: SceneObject): HLine
```

Draws a horizontal line from a start point that ends where it intersects
the target geometry. The nearest intersection (in either direction along
the X axis) is used.

| Parameter | Type | Description |
|-----------|------|-------------|
| `start` | [Point2DLike](/docs/api/types/point2dlike) | The start point |
| `target` | [SceneObject](/docs/api/types/scene-object) | The geometry to intersect with |

---

```ts
hLine(targetPlane: SceneObject | PlaneLike, distance: NumberParam): HLine
```

Draws a horizontal line on a specific plane.

| Parameter | Type | Description |
|-----------|------|-------------|
| `targetPlane` | [SceneObject](/docs/api/types/scene-object) \| [PlaneLike](/docs/api/types/plane-like) | The plane to draw on |
| `distance` | NumberParam | The line length |
