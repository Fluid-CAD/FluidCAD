...
```ts
.outerOf(...faces: (ISceneObject | FilterBuilderBase<Face>)[][]): this
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `...faces` | (ISceneObject \| FilterBuilderBase&lt;Face&gt;)[] | Face filter builders, or a scene object whose faces are the reference. *(optional)* |

### .notOuterOf()

```ts
.notOuterOf(...faces: (ISceneObject | FilterBuilderBase<Face>)[][]): this
```
...
```ts
.holeOf(...faces: (ISceneObject | FilterBuilderBase<Face>)[][]): this
```
...
```ts
.notHoleOf(...faces: (ISceneObject | FilterBuilderBase<Face>)[][]): this
```
