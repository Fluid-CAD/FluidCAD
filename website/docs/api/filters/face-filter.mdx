...
Excludes full cylindrical faces (see `cylinder`), optionally
...
bore cut open by a slot. A full bore is a `cylinder`.

| Parameter | Type | Description |
|-----------|------|-------------|
| `diameter` | number | Optional diameter to match (a fillet of radius r has diameter 2r). *(optional)* |
...
Excludes partial cylindrical faces (see `cylinderCurve`),
