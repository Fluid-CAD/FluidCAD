...
3. If a queued part declares [parameters](/docs/part/param), the next page shows one form per instance — two queued extrusions can get two lengths. See [Part parameters](./parameters).
...
Parameter overrides go in the second argument: `insert(plate, { Width: 120 })`. See [Part parameters](./parameters).
