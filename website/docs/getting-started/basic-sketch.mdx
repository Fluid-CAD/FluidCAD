...
4. The rectangle is the right size but not yet pinned: it could still slide. Press <kbd>Esc</kbd> to leave the tool, then click the origin, <kbd>Ctrl</kbd>+click the rectangle's top-left corner and <kbd>Ctrl</kbd>+click its bottom-left corner (<kbd>⌘</kbd> on a Mac). A plain click replaces the selection; holding <kbd>Ctrl</kbd> adds to it. The constraint toolbar appears under the sketch toolbar; click **Midpoint**. The origin becomes the midpoint of the left edge, so the rectangle is centred on the X axis with its left edge on the Y axis.
