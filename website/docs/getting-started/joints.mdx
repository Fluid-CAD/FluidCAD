...
2. Click the fixed leaf's `knuckle` triad (at the far end of its first knuckle). The slot fills with the connector's name and the dialog arms Connector B.
...
1. Click **Fastened**. Pick the fixed leaf's `pin` connector (the end of the tube) as A and the pin's `shaft` connector as B.
