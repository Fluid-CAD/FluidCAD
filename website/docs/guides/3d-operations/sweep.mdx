...
`.extend()` continues the swept solid past the end of the path, running straight along the path's tangent at that end. It's useful for lead-ins and run-outs — for example, extending a pipe straight past its last bend:

<CodeBlock language="js">{sweepExtend}</CodeBlock>
