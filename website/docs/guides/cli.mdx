...
The command refuses to run if `init.js` already exists, so it can't clobber a project. `test.fluid.js`, `jsconfig.json` and `fluidcad.json` are only written when they're missing, so re-running it in a half-set-up folder fills in the gaps.
...
See [Project configuration](/docs/getting-started/project-configuration) for what `fluidcad.json` holds.
...
extensions](/docs/getting-started/editor-setup) drive the same server if you'd
