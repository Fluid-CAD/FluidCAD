...
Rotate sketch geometry by an angle about an **explicit center point**:

```js
sketch("xy", () => {
    const c = circle([40, 0], 20)
    rotate(30, [0, 0], c)         // rotate 30° about the origin
    // rotate(30, [0, 0], true, c)  — rotate a COPY instead, keep the original
})
```

The 2D form is `rotate(angle, center, [copy,] ...targets)` — the center is required in a constraint sketch. Rotation happens in the sketch plane (about the plane normal at the given point). With no targets, the whole sketch's geometry rotates.
...
- `rotate(angle, center)` inside a sketch already rotates around the sketch's local Z at `center` — no axis argument, no `local()` needed.
