...
<UiFigure
  src="/img/docs/ui/constraint-bar-distance.png"
  alt="The constraint bar with a line picked and the Dimension button highlighted"
  rings={[{x: 77.5, y: 11.6, w: 6.9, h: 44.2}]}
  caption="A line picked: Dimension is enabled. It is the one button that is always available; with nothing picked it arms and waits for two picks."
/>

<ModeTable columns={[
  {title: 'Before', image: '/img/docs/sketching/dimension-linear-before.png', description: 'The base is drawn 70 long and the rise 25 tall, with no dimensions.'},
  {title: 'After', code: 'distance(base.start(), base.end(), 100)', image: '/img/docs/sketching/dimension-linear.png', description: 'Stretched to 100 × 45 by the two dimensions, whose labels stay on screen.'},
]} />
