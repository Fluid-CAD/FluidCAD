...
1. Click **Mirror** on the sketch toolbar (shortcut **M**). The Mirror dialog opens with a **Geometry** slot and a **Mirror line** slot.
2. Click the sketch edges to mirror; they show as chips. Any edge stands for its whole primitive — one side of a rectangle picks the rectangle.
3. Pick the **Mirror line**: click the slot, then click a sketched line in the viewport (a [guide](/docs/sketching/tools/guides) works too), or click the sketch's own X or Y axis line, written as `xAxis()` / `yAxis()`. The reflection previews in the viewport as you go.
4. **Apply**.
