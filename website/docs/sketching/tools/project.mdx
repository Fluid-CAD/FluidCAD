...
A sketch that is still on screen — a layout sketch, or one marked `.reusable()` — can be picked as a whole: click any of its curves while the Project dialog is open and the chip names the sketch's variable. Apply writes the sketch as a bare argument, binding the statement to a `const` if it has none yet:
