...
`rotate()` works on solids only. Sketch geometry is oriented with constraints instead — [`angle()`](/docs/sketching/constraints/dimensional/angle), [`horizontal()`](/docs/sketching/constraints/positional/horizontal) and [`vertical()`](/docs/sketching/constraints/positional/vertical) pin a line's direction, and the solver turns the rest of the profile with it. Calling `rotate()` inside `sketch(...)` is an error.
