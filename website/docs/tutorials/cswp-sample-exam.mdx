...
- `sketch(cylBody1.startFaces(), () => circle(E))` sketches a smaller circle of diameter `E` (41mm) on the pipe's far end face
...
- `sketch(corner.endFaces(), () => circle(15))` sketches a 15mm radius circle on the top face of the corner block, and `cut()` with no depth argument cuts through the entire block
- `sketch(corner.endFaces(), () => circle(30))` adds a larger 30mm radius circle on the same face, and `cut(10)` cuts only 10mm deep — creating the counterbore recess
