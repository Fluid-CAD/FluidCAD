...
In this tutorial, you'll build a grooved storage box based on the [original design by Too Tall Toby](https://www.youtube.com/watch?v=0JJH-FsHuq8). It covers constraint-sketched rounded rectangles, shelling, filleting internal edges, face intersection sketches, thin extrusions, and linear repeat patterns.
...
- **`line`** — draws a sketch line between two guess points
- **`fluidcad/constraints`** — the constraint statements that pin sketch geometry: `coincident` joins endpoints, `horizontal`/`vertical` align lines with the axes, `fix` anchors a point, `distance` dimensions a length
...
The third argument `true` on `sketch(...)` makes this a **constraint sketch**: the four `line(...)` statements are guesses, and the constraint statements pin them down exactly. The `coincident` calls join the endpoints into a closed loop, `horizontal`/`vertical` square it up, `fix` anchors the bottom-left corner at `[-85, -50]` so the 170 x 100 rectangle is centered on the origin, and the two `distance` dimensions set its size. With those in place the sketch is fully constrained — every line is pinned to exactly one position. `fillet(18, b, r, t, l)` then rounds all four corners of the loop with an 18-unit radius. `extrude(23.6)` pushes the shape up into a solid block. We store the result in `e` so we can reference its faces later.
...
- **Constraint sketching** — declaring lines as guesses and pinning them with `coincident`, `horizontal`/`vertical`, `fix`, and `distance` until the sketch is fully constrained
- **`fillet()` inside a sketch** — rounding the corners of a closed 2D loop
