...
export const fixedLeaf = part("Box hinge — fixed leaf", () => {
sketch("xy", () => {
const b = line([-leafReach,-barrelDiameter/2],[-rootClearance,-barrelDiameter/2]);
const r = line([-rootClearance,-barrelDiameter/2],[-rootClearance,-barrelDiameter/2+plateThickness]);
const t = line([-rootClearance,-barrelDiameter/2+plateThickness],[-leafReach,-barrelDiameter/2+plateThickness]);
const l = line([-leafReach,-barrelDiameter/2+plateThickness],[-leafReach,-barrelDiameter/2]);
...
fix(b.start(),[-leafReach,-barrelDiameter/2]);
distance(b.start(),b.end(),(-rootClearance)-(-leafReach));
distance(r.start(),r.end(),(-barrelDiameter/2+plateThickness)-(-barrelDiameter/2));
});
extrude(hingeLength);
sketch(plane("xy", axialClearance/2), () => {
const b = line([-rootClearance,-barrelDiameter/2],[0,-barrelDiameter/2]);
const r = line([0,-barrelDiameter/2],[0,-barrelDiameter/2+plateThickness]);
const t = line([0,-barrelDiameter/2+plateThickness],[-rootClearance,-barrelDiameter/2+plateThickness]);
...
distance(b.start(),b.end(),(0)-(-rootClearance));
distance(r.start(),r.end(),(-barrelDiameter/2+plateThickness)-(-barrelDiameter/2));
});
...
repeat("linear","z",{count:3,offset:2*knucklePitch},web,knuckle);
...
color("#B8C9B0");
chamfer(countersinkDepth, select(edge().circle(mountDiameter).farthest('y')));
});
